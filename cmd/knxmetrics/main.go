...
func main() {
	fx.New(
		...
		// viper configuration
		fx.Provide(stdfx.ConfigFile[knxmetrics.Config]("knxmetrics")),
		fx.Decorate(zerologfx.Decorator[knxmetrics.Config]),

		// cobra commands
		fx.Provide(
			stdfx.AutoRegister(stdfx.VersionCommand(version)),
			stdfx.AutoRegister(stdfx.ConfigCommand[knxmetrics.Config]),
...
func serverCommand(
	configProvider configfx.Provider[knxmetrics.Config],
	logger *zerolog.Logger,
) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "server",
		Short: "server - starts knxmetrics",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// create knxmetrics instance
			server, err := knxmetrics.New(cfg, logger)
			if err != nil {
				return err
			}

			// start knxmetrics using context
